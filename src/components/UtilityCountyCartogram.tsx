import { useMemo, useState, type ReactNode } from "react";
import { Wifi, Droplets, Zap, Search, TrendingUp } from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from "recharts";
import { Skeleton } from "@/components/ui/skeleton";
import {
  useMichiganEssentialServices,
  aggregateByCounty,
  groupPlacesByCounty,
  groupEnergyBurdenLeadByCounty,
  type CountyEntry,
  type PlaceEntry,
  type WaterSewageRateEntry,
  type BroadbandRateEntry,
  type EnergyBurdenLeadEntry,
} from "@/hooks/useMichiganEssentialServices";
import {
  useEnergyBurdenByCounty,
  useRateActionsByCounty,
  useRateHistoryByCounty,
  type EnergyBurdenEntry,
  type RateActionEntry,
  type RateHistoryEntry,
} from "@/hooks/useEnergyBurdenByCounty";

// ── Grid layout ──────────────────────────────────────────────────────────────
// Same technique as PurplBook.tsx's AtlasView: hand-derived cartogram grid,
// plain inline SVG. Rank-based grid (latitude bands -> consecutive integer
// columns) — see 2026-07-18 session notes for why this replaced direct
// centroid scaling. 16 rows x 11 cols, zero position collisions, all 83
// counties present.
//
// KNOWN GAP (deferred, per 2026-08-02 discussion): there is currently no
// visual break between the Upper and Lower Peninsulas — nothing in the grid
// signals UP vs. LP. Agreed to handle this as a separate structural pass
// (a labeled gap/divider row) once the underlying data issues below are
// settled, since color alone can't carry that information.

const GRID: Record<string, [number, number]> = {
  Alcona: [7, 9], Alger: [2, 5], Allegan: [13, 4], Alpena: [6, 9],
  Antrim: [6, 6], Arenac: [9, 8], Baraga: [1, 3], Barry: [13, 5],
  Bay: [10, 9], Benzie: [7, 4], Berrien: [15, 3], Branch: [15, 6],
  Calhoun: [14, 6], Cass: [15, 4], Charlevoix: [5, 5], Cheboygan: [5, 7],
  Chippewa: [3, 7], Clare: [9, 6], Clinton: [12, 7], Crawford: [7, 7],
  Delta: [4, 5], Dickinson: [4, 4], Eaton: [13, 6], Emmet: [5, 6],
  Genesee: [12, 9], Gladwin: [9, 7], Gogebic: [2, 3], "Grand Traverse": [7, 5],
  Gratiot: [11, 6], Hillsdale: [15, 7], Houghton: [1, 2], Huron: [9, 9],
  Ingham: [13, 7], Ionia: [12, 6], Iosco: [8, 9], Iron: [3, 4],
  Isabella: [10, 7], Jackson: [14, 7], Kalamazoo: [14, 5], Kalkaska: [7, 6],
  Kent: [12, 5], Keweenaw: [0, 2], Lake: [9, 4], Lapeer: [11, 9],
  Leelanau: [6, 5], Lenawee: [15, 8], Livingston: [13, 8], Luce: [2, 6],
  Mackinac: [3, 6], Macomb: [13, 10], Manistee: [8, 4], Marquette: [2, 4],
  Mason: [9, 3], Mecosta: [10, 6], Menominee: [5, 4], Midland: [10, 8],
  Missaukee: [8, 6], Monroe: [15, 9], Montcalm: [11, 5], Montmorency: [6, 8],
  Muskegon: [11, 4], Newaygo: [10, 5], Oakland: [13, 9], Oceana: [10, 4],
  Ogemaw: [8, 8], Ontonagon: [1, 1], Osceola: [9, 5], Oscoda: [7, 8],
  Otsego: [6, 7], Ottawa: [12, 4], "Presque Isle": [5, 8], Roscommon: [8, 7],
  Saginaw: [11, 7], Sanilac: [11, 10], Schoolcraft: [3, 5], Shiawassee: [12, 8],
  "St. Clair": [12, 10], "St. Joseph": [15, 5], Tuscola: [11, 8], "Van Buren": [14, 4],
  Washtenaw: [14, 8], Wayne: [14, 9], Wexford: [8, 5],
};

const ALL_COUNTIES = Object.keys(GRID).sort();
const CELL = 42, GAP = 5;
const GRID_ROWS = Math.max(...Object.values(GRID).map(([r]) => r)) + 1;
const GRID_COLS = Math.max(...Object.values(GRID).map(([, c]) => c)) + 1;

const ABBR: Record<string, string> = {
  Alcona: "ALC", Alger: "ALG", Allegan: "ALL", Alpena: "ALP", Antrim: "ANT",
  Arenac: "ARE", Baraga: "BRG", Barry: "BAR", Bay: "BAY", Benzie: "BEN",
  Berrien: "BER", Branch: "BRA", Calhoun: "CAL", Cass: "CAS", Charlevoix: "CHA",
  Cheboygan: "CHE", Chippewa: "CHI", Clare: "CLA", Clinton: "CLI", Crawford: "CRA",
  Delta: "DEL", Dickinson: "DIC", Eaton: "EAT", Emmet: "EMM", Genesee: "GEN",
  Gladwin: "GLA", Gogebic: "GOG", "Grand Traverse": "GT", Gratiot: "GRA",
  Hillsdale: "HIL", Houghton: "HOU", Huron: "HUR", Ingham: "ING", Ionia: "ION",
  Iosco: "IOS", Iron: "IRO", Isabella: "ISA", Jackson: "JAC", Kalamazoo: "KZO",
  Kalkaska: "KLK", Kent: "KEN", Keweenaw: "KEW", Lake: "LAK", Lapeer: "LAP",
  Leelanau: "LEE", Lenawee: "LEN", Livingston: "LIV", Luce: "LUC", Mackinac: "MAK",
  Macomb: "MAC", Manistee: "MAN", Marquette: "MAR", Mason: "MAS", Mecosta: "MEC",
  Menominee: "MEN", Midland: "MID", Missaukee: "MIS", Monroe: "MOE", Montcalm: "MTC",
  Montmorency: "MTM", Muskegon: "MUS", Newaygo: "NEW", Oakland: "OAK", Oceana: "OCE",
  Ogemaw: "OGE", Ontonagon: "ONT", Osceola: "OSC", Oscoda: "OSD", Otsego: "OTS",
  Ottawa: "OTT", "Presque Isle": "PI", Roscommon: "ROS", Saginaw: "SAG",
  Sanilac: "SAN", Schoolcraft: "SCH", Shiawassee: "SHI", "St. Clair": "SC",
  "St. Joseph": "SJ", Tuscola: "TUS", "Van Buren": "VB", Washtenaw: "WAS",
  Wayne: "WAY", Wexford: "WEX",
};

type LayerKey = "broadband" | "water";
type ViewKey = "map" | "list";
type CategoryKey = "both" | "water" | "broadband" | "none";

// ── Broadband: excluding satellite from the headline number ────────────────
// "Any Technology" in the raw FCC BDC data folds in satellite (Starlink et
// al.), which reads as ~100% in nearly every Michigan county/place — it is
// not a meaningful signal of terrestrial accessibility and was producing a
// map/number that looked uniformly excellent everywhere. Confirmed 2026-08-02
// against Ontonagon County specifically: "Any Technology" = 100% in all 9
// sampled places there, while Fixed Wireless is 0% in every one of them and
// Fiber is 0% in three of nine — the real, county-specific story that "Any
// Technology" was masking.
//
// Fix: every broadband number surfaced by this component (map category
// presence, the List view's bar/value, and the county detail card's
// headline) is now computed here directly from the raw per-technology
// records, taking the BEST of the three genuinely terrestrial technologies
// (Fiber, Cable, All Fixed Wireless) per county. "Any Technology" and "All
// Satellite" are never used as the headline; satellite is shown only as an
// explicit, separately-labeled footnote in the detail card so it isn't lost,
// just no longer allowed to stand in for real terrestrial coverage.
//
// NOTE: this only fixes what's computed in THIS component. The hook's own
// `aggregateByCounty` (from useMichiganEssentialServices) may still expose a
// separate, satellite-inclusive `broadbandAvailabilityPct` field — that
// field is intentionally NOT used anywhere below. If other parts of the app
// read that hook field directly, they'll need the same fix applied there.
const FIXED_TERRESTRIAL_TECHS = new Set(["Fiber", "Cable", "All Fixed Wireless"]);

function terrestrialCompositeByCounty(rates: BroadbandRateEntry[]): Map<string, number> {
  const byCounty = new Map<string, Map<string, { total: number; count: number }>>();
  for (const r of rates) {
    if (r.availabilityPctOfZip === null || !FIXED_TERRESTRIAL_TECHS.has(r.technology ?? "")) continue;
    if (!byCounty.has(r.county)) byCounty.set(r.county, new Map());
    const byTech = byCounty.get(r.county)!;
    const entry = byTech.get(r.technology!) ?? { total: 0, count: 0 };
    entry.total += r.availabilityPctOfZip;
    entry.count += 1;
    byTech.set(r.technology!, entry);
  }
  const result = new Map<string, number>();
  for (const [county, byTech] of byCounty) {
    let best = 0;
    for (const { total, count } of byTech.values()) best = Math.max(best, total / count);
    result.set(county, Math.round(best));
  }
  return result;
}

const LAYER_LABEL: Record<LayerKey, string> = {
  broadband: "Broadband Coverage (Terrestrial)",
  water: "Water Affordability Burden",
};
const LAYER_ICON: Record<LayerKey, typeof Wifi> = {
  broadband: Wifi,
  water: Droplets,
};
const VIEW_LABEL: Record<ViewKey, string> = { map: "Map", list: "County List" };

// ── Map coloring: categorical, not continuous ───────────────────────────────
// Previously the map colored counties along a single continuous ramp driven
// by whichever layer (water OR broadband) was toggled on, hiding the other
// layer entirely and making color depend on which button happened to be
// pressed rather than on what data actually exists for that county. Per
// 2026-08-02 discussion, the map now shows a single, fixed categorical view
// of DATA AVAILABILITY across both categories at once — the layer toggle
// (below) still exists for the List view's bar/value, but no longer drives
// the map's fill.
//
// Energy is deliberately excluded from these categories: verified directly
// against the live database that energy_burden_by_race has geography_id =
// null on every row, so it's genuinely statewide reference data, not
// something with a per-county presence/absence to color by. It's shown as
// always-present context in every county's detail panel regardless of color.
// (Note for later: water & sewage could arguably move toward this same
// "always-present statewide context" treatment rather than a hard
// presence/absence category — flagged for a future pass, not done here.)
//
// Colors are the Liberation Caucus palette. Gold/terracotta match values
// already used elsewhere in this file (terracotta = the existing selected-
// county stroke color); forest green is the Pan-African-palette green
// mentioned in brand notes. Confirm against the actual Tailwind config if
// these read as off from the brand's defined hex values.
const CATEGORY_COLOR: Record<CategoryKey, string> = {
  both: "#C9A438",       // gold — water & sewage AND broadband both sampled
  water: "#A8442C",      // terracotta — water & sewage only
  broadband: "#2F6B45",  // forest green — broadband only
  none: "#1a1512",       // near-black — no data yet (a gap, not a "good" score)
};
const CATEGORY_TEXT_COLOR: Record<CategoryKey, string> = {
  both: "#1f2937",
  water: "#f4ecd8",
  broadband: "#f4ecd8",
  none: "#f4ecd8",
};
const CATEGORY_LABEL: Record<CategoryKey, string> = {
  both: "Water & sewage + broadband",
  water: "Water & sewage only",
  broadband: "Broadband only",
  none: "No data yet",
};

function barColor(layer: LayerKey): string {
  return layer === "broadband" ? "hsl(152 55% 55%)" : "hsl(14 65% 58%)";
}

export default function UtilityCountyCartogram() {
  const [view, setView] = useState<ViewKey>("map");
  const [layer, setLayer] = useState<LayerKey>("water");
  const [openCounty, setOpenCounty] = useState<string | null>(null);
  const [hovered, setHovered] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  const { data: servicesData, isLoading: servicesLoading, isError: servicesError } = useMichiganEssentialServices();
  const { data: energyRows, isLoading: energyLoading } = useEnergyBurdenByCounty();

  const isLoading = servicesLoading || energyLoading;
  const isError = servicesError;

  const countiesByName = useMemo(() => {
    const map = new Map<string, CountyEntry>();
    for (const c of servicesData?.counties ?? []) map.set(c.name, c);
    return map;
  }, [servicesData]);

  const countyAggregates = useMemo(
    () => aggregateByCounty(servicesData?.waterSewageRates ?? [], servicesData?.broadbandRates ?? []),
    [servicesData]
  );

  const terrestrialBroadbandByCounty = useMemo(
    () => terrestrialCompositeByCounty(servicesData?.broadbandRates ?? []),
    [servicesData]
  );

  const placesByCounty = useMemo(
    () => groupPlacesByCounty(servicesData?.places ?? []),
    [servicesData]
  );

  const energyBurdenLeadByCounty = useMemo(
    () => groupEnergyBurdenLeadByCounty(servicesData?.energyBurdenLead ?? []),
    [servicesData]
  );

  function metricValue(countyName: string, ly: LayerKey): number | null {
    if (ly === "broadband") return terrestrialBroadbandByCounty.get(countyName) ?? null;
    return countyAggregates.get(countyName)?.waterAffordabilityRatioPct ?? null;
  }

  function categoryFor(countyName: string): CategoryKey {
    const hasWater = metricValue(countyName, "water") !== null;
    const hasBroadband = metricValue(countyName, "broadband") !== null;
    if (hasWater && hasBroadband) return "both";
    if (hasWater) return "water";
    if (hasBroadband) return "broadband";
    return "none";
  }

  const filteredCounties = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return ALL_COUNTIES;
    return ALL_COUNTIES.filter(
      (n) =>
        n.toLowerCase().includes(q) ||
        (placesByCounty.get(n) ?? []).some((p) => p.name.toLowerCase().includes(q))
    );
  }, [query, placesByCounty]);

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 mb-4 pb-0">
        <div className="flex">
          {(Object.keys(VIEW_LABEL) as ViewKey[]).map((key) => (
            <button
              key={key}
              onClick={() => setView(key)}
              className={`px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 -mb-px transition-colors ${
                view === key
                  ? "text-gray-900 border-liberation-gold"
                  : "text-gray-400 border-transparent hover:text-gray-700"
              }`}
              aria-current={view === key ? "true" : undefined}
            >
              {VIEW_LABEL[key]}
            </button>
          ))}
        </div>
        <div className="relative mb-2">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Find a county…"
            aria-label="Search counties"
            className="pl-7 pr-3 py-1.5 text-xs rounded-md bg-gray-100 border border-gray-300 text-gray-900 placeholder:text-gray-400 w-40 focus:outline-none focus:border-liberation-gold/50"
          />
        </div>
      </div>

      {/* Layer toggle: only meaningful for the List view now, since the map
          uses fixed categorical coloring regardless of layer selection. */}
      {view === "list" && (
        <div className="mb-4">
          <div className="flex gap-2 flex-wrap">
            {(Object.keys(LAYER_LABEL) as LayerKey[]).map((key) => {
              const Icon = LAYER_ICON[key];
              const active = layer === key;
              return (
                <button
                  key={key}
                  onClick={() => setLayer(key)}
                  aria-pressed={active}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${
                    active
                      ? "bg-liberation-gold text-liberation-dark border-liberation-gold"
                      : "bg-transparent text-gray-600 border-gray-300 hover:border-gray-400"
                  }`}
                >
                  <Icon className="w-3.5 h-3.5" />
                  {LAYER_LABEL[key]}
                </button>
              );
            })}
          </div>
          <p className="text-[11px] text-gray-400 mt-1.5">
            Broadband coverage excludes satellite (near-universal statewide via services like Starlink) — see each county's detail for the satellite figure separately.
          </p>
        </div>
      )}

      {isLoading && (
        <div className="space-y-3" aria-busy="true" aria-label="Loading essential services data">
          <Skeleton className="h-[500px] w-full max-w-2xl rounded-lg" />
          <Skeleton className="h-4 w-2/3" />
          <Skeleton className="h-4 w-1/3" />
        </div>
      )}

      {isError && (
        <div role="alert" className="h-40 flex flex-col items-center justify-center gap-2 text-gray-500 text-sm border border-gray-200 rounded-lg">
          <span>Data couldn't load.</span>
          <button onClick={() => window.location.reload()} className="text-xs text-liberation-gold hover:underline">
            Try refreshing the page
          </button>
        </div>
      )}

      {!isLoading && !isError && view === "map" && (
        <MapView
          query={query}
          openCounty={openCounty}
          setOpenCounty={setOpenCounty}
          hovered={hovered}
          setHovered={setHovered}
          metricValue={metricValue}
          categoryFor={categoryFor}
        />
      )}

      {!isLoading && !isError && view === "list" && (
        <ListView
          layer={layer}
          counties={filteredCounties}
          openCounty={openCounty}
          setOpenCounty={setOpenCounty}
          metricValue={metricValue}
          countiesByName={countiesByName}
          waterRates={servicesData?.waterSewageRates ?? []}
          broadbandRates={servicesData?.broadbandRates ?? []}
          energyRows={energyRows ?? []}
          energyBurdenLead={servicesData?.energyBurdenLead ?? []}
          placesByCounty={placesByCounty}
        />
      )}

      {view === "map" && !isLoading && !isError && (
        <div className="border-t-2 border-gray-200 mt-1">
          {openCounty ? (
            <CountyDetailPanel
              countyName={openCounty}
              county={countiesByName.get(openCounty)}
              waterRates={(servicesData?.waterSewageRates ?? []).filter((w) => w.county === openCounty)}
              broadbandRates={(servicesData?.broadbandRates ?? []).filter((b) => b.county === openCounty)}
              energyRows={energyRows ?? []}
              leadRows={energyBurdenLeadByCounty.get(openCounty) ?? []}
              places={placesByCounty.get(openCounty) ?? []}
              onClose={() => setOpenCounty(null)}
            />
          ) : (
            <p className="text-left text-xs text-gray-400 py-6">
              Select any county above to see its water, energy, and broadband data.
            </p>
          )}
        </div>
      )}
    </div>
  );
}

// ── Map view ──────────────────────────────────────────────────────────────────

function MapView({
  query,
  openCounty,
  setOpenCounty,
  hovered,
  setHovered,
  metricValue,
  categoryFor,
}: {
  query: string;
  openCounty: string | null;
  setOpenCounty: (c: string | null) => void;
  hovered: string | null;
  setHovered: (c: string | null) => void;
  metricValue: (countyName: string, layer: LayerKey) => number | null;
  categoryFor: (countyName: string) => CategoryKey;
}) {
  const q = query.trim().toLowerCase();

  return (
    <>
      <svg
        viewBox={`0 0 ${GRID_COLS * (CELL + GAP)} ${GRID_ROWS * (CELL + GAP)}`}
        className="w-full max-w-2xl block"
        style={{ height: "auto", overflow: "visible" }}
        role="group"
        aria-label="Michigan county cartogram"
      >
        {Object.entries(GRID).map(([name, [r, c]]) => {
          const category = categoryFor(name);
          const isOpen = openCounty === name;
          const isHovered = hovered === name;
          const matches = !q || name.toLowerCase().includes(q);
          const x = c * (CELL + GAP);
          const y = r * (CELL + GAP);
          const cx = x + CELL / 2;
          const cy = y + CELL / 2;
          return (
            <g
              key={name}
              tabIndex={0}
              role="button"
              aria-label={`${name} County — ${CATEGORY_LABEL[category]}`}
              aria-pressed={isOpen}
              opacity={matches ? 1 : 0.2}
              style={{
                cursor: "pointer",
                outline: "none",
                transform: isHovered ? "scale(1.12)" : "scale(1)",
                transformOrigin: `${cx}px ${cy}px`,
                transition: "transform 0.15s ease",
              }}
              onClick={() => setOpenCounty(isOpen ? null : name)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
                  setOpenCounty(isOpen ? null : name);
                }
              }}
              onMouseEnter={() => setHovered(name)}
              onMouseLeave={() => setHovered(null)}
              onFocus={() => setHovered(name)}
              onBlur={() => setHovered(null)}
            >
              <rect
                x={x} y={y} width={CELL} height={CELL} rx={6}
                fill={CATEGORY_COLOR[category]}
                stroke={isOpen ? "#1f2937" : "#e5e7eb"}
                strokeWidth={isOpen ? 2.5 : 0.75}
                style={{ transition: "stroke 0.1s ease" }}
              />
              <text
                x={cx} y={cy}
                textAnchor="middle"
                dominantBaseline="central"
                fontFamily="ui-monospace, monospace"
                fontWeight={600}
                fontSize={ABBR[name]?.length > 2 ? 13 : 15}
                fill={CATEGORY_TEXT_COLOR[category]}
                style={{ pointerEvents: "none" }}
              >
                {ABBR[name] ?? name.slice(0, 3).toUpperCase()}
              </text>
            </g>
          );
        })}
      </svg>

      <div className="text-left text-xs text-gray-400 mt-2 h-4" aria-live="polite">
        {hovered ? (
          <span>
            <strong className="text-gray-800">{hovered} County</strong>
            {" — "}
            {CATEGORY_LABEL[categoryFor(hovered)]}
            {(() => {
              const b = metricValue(hovered, "broadband");
              const w = metricValue(hovered, "water");
              const parts: string[] = [];
              if (b !== null) parts.push(`broadband ${b}% terrestrial`);
              if (w !== null) parts.push(`water ${w}% of income`);
              return parts.length > 0 ? ` (${parts.join(", ")})` : "";
            })()}
          </span>
        ) : (
          "Hover or tab to a county for its data · click or press Enter to open details below"
        )}
      </div>

      <div className="flex gap-4 flex-wrap text-[11px] text-gray-500 py-3">
        {(Object.keys(CATEGORY_LABEL) as CategoryKey[]).map((key) => (
          <span key={key} className="flex items-center gap-1.5">
            <span
              className="inline-block w-3 h-3 rounded-sm border border-gray-300"
              style={{ background: CATEGORY_COLOR[key] }}
            />
            {CATEGORY_LABEL[key]}
          </span>
        ))}
      </div>
    </>
  );
}

// ── County List (Ledger) view ──────────────────────────────────────────────────

function ListView({
  layer,
  counties,
  openCounty,
  setOpenCounty,
  metricValue,
  countiesByName,
  waterRates,
  broadbandRates,
  energyRows,
  energyBurdenLead,
  placesByCounty,
}: {
  layer: LayerKey;
  counties: string[];
  openCounty: string | null;
  setOpenCounty: (c: string | null) => void;
  metricValue: (countyName: string, layer: LayerKey) => number | null;
  countiesByName: Map<string, CountyEntry>;
  waterRates: WaterSewageRateEntry[];
  broadbandRates: BroadbandRateEntry[];
  energyRows: EnergyBurdenEntry[];
  energyBurdenLead: EnergyBurdenLeadEntry[];
  placesByCounty: Map<string, PlaceEntry[]>;
}) {
  const max = useMemo(() => {
    let m = 1;
    for (const name of counties) {
      const v = metricValue(name, layer);
      if (v !== null) m = Math.max(m, v);
    }
    return m;
  }, [counties, layer, metricValue]);

  if (counties.length === 0) {
    return <p className="text-xs text-gray-400 py-6">No counties or places match that search.</p>;
  }

  return (
    <div role="list" aria-label="Michigan counties">
      {counties.map((name) => {
        const value = metricValue(name, layer);
        const isEmpty = value === null;
        const widthPct = isEmpty ? 0 : Math.max((value / max) * 100, 2);
        const isOpen = openCounty === name;
        const places = placesByCounty.get(name) ?? [];

        return (
          <div key={name} role="listitem" className="border-b border-gray-200">
            <button
              onClick={() => setOpenCounty(isOpen ? null : name)}
              aria-expanded={isOpen}
              className="w-full flex items-center gap-3 py-2.5 text-left"
            >
              <span className={`flex-shrink-0 w-3.5 h-3.5 text-gray-400 transition-transform ${isOpen ? "rotate-90 text-liberation-gold" : ""}`}>
                ▸
              </span>
              <span className={`font-bold text-sm flex-shrink-0 w-28 ${isEmpty ? "text-gray-400" : "text-gray-900"}`}>
                {name}
                {places.length > 0 && (
                  <span className="ml-1.5 text-[10px] font-normal text-gray-400">({places.length})</span>
                )}
              </span>
              <span className="flex-1 h-1.5 rounded-full bg-gray-50 overflow-hidden max-w-[160px]">
                <span className="block h-full rounded-full" style={{ width: `${widthPct}%`, background: barColor(layer) }} />
              </span>
              <span className={`text-xs font-mono w-16 text-right flex-shrink-0 ${isEmpty ? "text-gray-400" : "text-gray-700"}`}>
                {isEmpty ? "—" : `${value}%`}
              </span>
            </button>
            {isOpen && (
              <CountyDetailPanel
                countyName={name}
                county={countiesByName.get(name)}
                waterRates={waterRates.filter((w) => w.county === name)}
                broadbandRates={broadbandRates.filter((b) => b.county === name)}
                energyRows={energyRows}
                leadRows={energyBurdenLead.filter((r) => r.county === name)}
                places={places}
                onClose={() => setOpenCounty(null)}
                compact
              />
            )}
          </div>
        );
      })}
    </div>
  );
}

// ── County detail panel ──────────────────────────────────────────────────────

function TruncatedList<T>({
  items,
  render,
  keyFn,
  initialCount = 5,
}: {
  items: T[];
  render: (item: T) => ReactNode;
  keyFn: (item: T) => string;
  initialCount?: number;
}) {
  const [expanded, setExpanded] = useState(false);
  const visible = expanded ? items : items.slice(0, initialCount);
  const hiddenCount = items.length - initialCount;

  return (
    <>
      <ul className="space-y-1 text-xs text-gray-600">
        {visible.map((item) => (
          <li key={keyFn(item)}>{render(item)}</li>
        ))}
      </ul>
      {!expanded && hiddenCount > 0 && (
        <button onClick={() => setExpanded(true)} className="text-[11px] text-liberation-gold hover:underline mt-1">
          +{hiddenCount} more
        </button>
      )}
    </>
  );
}

// ── Metric bar list ───────────────────────────────────────────────────────────
// A structured label + horizontal bar + value row, reused for two things that
// don't belong in a pie chart because their values are INDEPENDENT figures
// that don't sum to a meaningful 100%: (1) broadband technology coverage,
// where fiber/cable/fixed-wireless footprints overlap rather than dividing up
// a whole; and (2) energy burden by race, where each group's burden % is its
// own independent statistic. This mirrors the same bold-label +
// structured-metric visual language already used in the Energy Providers &
// Rate Changes list below and in the main county List view's row bars, so
// it's intentional and consistent rather than a bar chart invented from
// nothing.
function MetricBarList({
  items,
  barHexColor,
  emptyLabel = "No data.",
}: {
  items: Array<{ label: string; pct: number; sublabel?: string }>;
  barHexColor: string;
  emptyLabel?: string;
}) {
  if (items.length === 0) return <p className="text-xs text-gray-400">{emptyLabel}</p>;
  const max = Math.max(...items.map((i) => i.pct), 1);
  return (
    <ul className="space-y-2">
      {items.map((i) => (
        <li key={i.label}>
          <div className="flex items-baseline justify-between gap-2 mb-1">
            <span className="text-xs font-semibold text-gray-800 leading-snug">
              {i.label}
              {i.sublabel && <span className="text-gray-400 font-normal"> {i.sublabel}</span>}
            </span>
            <span className="text-[11px] font-mono text-gray-600 flex-shrink-0">{i.pct}%</span>
          </div>
          <span className="block h-1.5 rounded-full bg-gray-200 overflow-hidden">
            <span
              className="block h-full rounded-full"
              style={{ width: `${Math.min(100, (i.pct / max) * 100)}%`, background: barHexColor }}
            />
          </span>
        </li>
      ))}
    </ul>
  );
}

// ── Mini pie chart ────────────────────────────────────────────────────────────
// Kept for the two places where slices genuinely ARE parts of a whole
// (sum to ~100%): water bill composition, and population share by race.
const PIE_COLORS = ["hsl(152 45% 58%)", "hsl(14 65% 62%)", "hsl(40 55% 62%)", "hsl(210 45% 62%)", "hsl(280 35% 62%)"];

function MiniPie({ data }: { data: Array<{ name: string; value: number }> }) {
  if (data.length === 0 || data.every((d) => d.value === 0)) {
    return <p className="text-xs text-gray-400">Not enough data to chart.</p>;
  }
  return (
    <div className="flex items-center gap-3">
      <div style={{ width: 72, height: 72 }} className="flex-shrink-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="name" innerRadius={18} outerRadius={34} paddingAngle={2}>
              {data.map((_, i) => (
                <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} stroke="none" />
              ))}
            </Pie>
            <Tooltip
              formatter={(value: number, name: string) => [`${value}%`, name]}
              contentStyle={{ background: "#1a1512", border: "1px solid rgba(244,236,216,0.15)", borderRadius: 6, fontSize: 11 }}
              itemStyle={{ color: "#f4ecd8" }}
            />
          </PieChart>
        </ResponsiveContainer>
      </div>
      <ul className="text-[11px] text-gray-600 space-y-0.5">
        {data.map((d, i) => (
          <li key={d.name} className="flex items-center gap-1.5">
            <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ background: PIE_COLORS[i % PIE_COLORS.length] }} />
            {d.name}: {d.value}%
          </li>
        ))}
      </ul>
    </div>
  );
}

function CountyDetailPanel({
  countyName,
  county,
  waterRates,
  broadbandRates,
  energyRows,
  leadRows,
  places,
  onClose,
  compact = false,
}: {
  countyName: string;
  county: CountyEntry | undefined;
  waterRates: WaterSewageRateEntry[];
  broadbandRates: BroadbandRateEntry[];
  energyRows: EnergyBurdenEntry[];
  leadRows: EnergyBurdenLeadEntry[];
  places: PlaceEntry[];
  onClose: () => void;
  compact?: boolean;
}) {
  const { data: allRateActions } = useRateActionsByCounty() as { data: RateActionEntry[] | undefined };
  const { data: allRateHistory } = useRateHistoryByCounty() as { data: RateHistoryEntry[] | undefined };

  const countyRateActions = useMemo(
    () => (allRateActions ?? []).filter((a: RateActionEntry) => a.geography === countyName),
    [allRateActions, countyName]
  );

  const relevantUtilities = useMemo(
    () => new Set(countyRateActions.map((a: RateActionEntry) => a.utility).filter(Boolean)),
    [countyRateActions]
  );
  const utilityRateHistory = useMemo(
    () =>
      (allRateHistory ?? [])
        .filter((h: RateHistoryEntry) => relevantUtilities.has(h.utility))
        .sort((a: RateHistoryEntry, b: RateHistoryEntry) => (a.year ?? 0) - (b.year ?? 0)),
    [allRateHistory, relevantUtilities]
  );

  // Broadband: average availability per technology, split into terrestrial
  // (the headline), satellite (a footnote — near-universal, not a meaningful
  // differentiator), and mobile (its own small section, if present — mobile
  // rows measure LAND AREA covered, not housing units, so they are never
  // mixed into the terrestrial housing-unit figures above).
  const broadbandByTech = useMemo(() => {
    const sums = new Map<string, { total: number; count: number }>();
    for (const r of broadbandRates) {
      if (r.availabilityPctOfZip === null) continue;
      const tech = r.technology || "Unknown";
      const entry = sums.get(tech) ?? { total: 0, count: 0 };
      entry.total += r.availabilityPctOfZip;
      entry.count += 1;
      sums.set(tech, entry);
    }
    return Array.from(sums.entries()).map(([tech, { total, count }]) => ({
      tech,
      avgPct: Math.round(total / count),
    }));
  }, [broadbandRates]);

  const terrestrialTechs = broadbandByTech.filter((t) => FIXED_TERRESTRIAL_TECHS.has(t.tech));
  const satelliteTech = broadbandByTech.find((t) => t.tech === "All Satellite");
  const mobileTechs = broadbandByTech.filter((t) => t.tech.startsWith("Mobile"));
  const terrestrialHeadline =
    terrestrialTechs.length > 0 ? Math.max(...terrestrialTechs.map((t) => t.avgPct)) : null;

  const waterPie = useMemo(() => {
    const withBoth = waterRates.filter(
      (r) => r.estimatedTypicalMonthlyBill !== null && r.customerChargeMonthly !== null && r.estimatedTypicalMonthlyBill > 0
    );
    if (withBoth.length === 0) return [];
    const avgBill = withBoth.reduce((s, r) => s + (r.estimatedTypicalMonthlyBill ?? 0), 0) / withBoth.length;
    const avgFixed = withBoth.reduce((s, r) => s + (r.customerChargeMonthly ?? 0), 0) / withBoth.length;
    const fixedPct = Math.max(0, Math.min(100, Math.round((avgFixed / avgBill) * 100)));
    return [
      { name: "Fixed charge", value: fixedPct },
      { name: "Est. usage charge", value: 100 - fixedPct },
    ];
  }, [waterRates]);

  // Real per-county energy burden (DOE LEAD Tool) — genuinely county-
  // specific, unlike the statewide ACEEE race reference below. 'ALL' is the
  // weighted county-wide figure across all income levels; the other five
  // brackets show the income-tier breakdown (e.g. households below the
  // federal poverty line vs. households at 400%+ of it), sorted low-to-high
  // income by groupEnergyBurdenLeadByCounty.
  const leadHeadline = leadRows.find((r) => r.fplBracket === "ALL")?.energyBurdenPct ?? null;
  const leadBracketItems = useMemo(
    () =>
      leadRows
        .filter((r) => r.fplBracket !== "ALL" && r.energyBurdenPct !== null)
        .map((r) => ({
          label: r.fplBracket + " of poverty line",
          pct: r.energyBurdenPct as number,
        })),
    [leadRows]
  );

  // Energy burden by race: previously a plain text list of independent
  // percentages (which don't sum to 100, so were never suited to a pie).
  // Now a bar list — same structured label + bar + value treatment used for
  // broadband above and for the Rate Changes list below, so a reader can
  // compare groups at a glance instead of reading numbers off a list.
  const burdenBarItems = useMemo(
    () =>
      energyRows
        .filter((r) => r.medianBurdenPct !== null)
        .map((r) => ({
          label: r.racialGroup,
          pct: r.medianBurdenPct as number,
          sublabel: r.dataYear ? `(${r.dataYear})` : undefined,
        })),
    [energyRows]
  );

  return (
    <div className={compact ? "pb-4 pl-6" : "py-4"} aria-live="polite">
      {!compact && (
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-semibold text-gray-900">{countyName} County</h4>
          <button onClick={onClose} className="text-xs text-gray-400 hover:text-gray-800">
            Close ✕
          </button>
        </div>
      )}

      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Wifi className="w-3.5 h-3.5" /> Broadband
          </div>
          {broadbandRates.length > 0 ? (
            <>
              {terrestrialHeadline !== null && (
                <div className="text-2xl font-bold text-gray-900 mb-1">
                  {terrestrialHeadline}%
                  <span className="text-[10px] font-normal text-gray-400 ml-1.5">best terrestrial option, avg.</span>
                </div>
              )}
              <MetricBarList
                items={terrestrialTechs.map((t) => ({ label: t.tech, pct: t.avgPct }))}
                barHexColor="#2F6B45"
                emptyLabel="No terrestrial (fiber/cable/fixed wireless) data sampled."
              />
              {satelliteTech && (
                <p className="text-[11px] text-gray-400 mt-2 pt-2 border-t border-gray-200">
                  Satellite: {satelliteTech.avgPct}% — omitted from the figure above; typically costlier and higher-latency than terrestrial options, and near-universal statewide.
                </p>
              )}
              {mobileTechs.length > 0 && (
                <div className="mt-2 pt-2 border-t border-gray-200">
                  <div className="text-[11px] text-gray-500 mb-1">
                    Mobile coverage (share of land area, not housing units):
                  </div>
                  <MetricBarList
                    items={mobileTechs.map((t) => ({ label: t.tech.replace("Mobile ", ""), pct: t.avgPct }))}
                    barHexColor="hsl(210 45% 55%)"
                  />
                </div>
              )}
            </>
          ) : (
            <p className="text-xs text-gray-400">Not yet sampled in this county.</p>
          )}
        </div>

        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Droplets className="w-3.5 h-3.5" /> Water & Sewage
          </div>
          {waterRates.length > 0 ? (
            <>
              <MiniPie data={waterPie} />
              <div className="mt-2">
                <TruncatedList
                  items={waterRates}
                  keyFn={(r) => r.id}
                  initialCount={3}
                  render={(r) => (
                    <>
                      {r.provider} ({r.municipalityServiceArea || "—"})
                      {r.estimatedTypicalMonthlyBill !== null && ` — $${r.estimatedTypicalMonthlyBill}/mo`}
                      {r.affordabilityRatioPct !== null && `, ${r.affordabilityRatioPct}% of income`}
                    </>
                  )}
                />
              </div>
            </>
          ) : (
            <p className="text-xs text-gray-400">Not yet sampled in this county.</p>
          )}
        </div>

        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Zap className="w-3.5 h-3.5" /> Energy Burden
          </div>
          {leadHeadline !== null ? (
            <>
              <div className="text-2xl font-bold text-gray-900 mb-1">
                {leadHeadline}%
                <span className="text-[10px] font-normal text-gray-400 ml-1.5">of income, county-wide avg.</span>
              </div>
              <MetricBarList items={leadBracketItems} barHexColor="#A8442C" emptyLabel="No income-tier breakdown available." />
              <p className="text-[11px] text-gray-400 mt-2 pt-2 border-t border-gray-200">
                {countyName} County, DOE LEAD Tool (2022 ACS, calibrated to EIA utility filings) — real county data, weighted across income tiers shown above.
              </p>
            </>
          ) : (
            <p className="text-xs text-gray-400">County-level LEAD data unavailable.</p>
          )}
          {burdenBarItems.length > 0 && (
            <div className="mt-2 pt-2 border-t border-gray-200">
              <div className="text-[11px] text-gray-500 mb-1.5">
                By race, statewide reference (not specific to {countyName} — this figure isn't tracked at the county level in any public source):
              </div>
              <MetricBarList items={burdenBarItems} barHexColor="#A8442C" />
            </div>
          )}
        </div>
      </div>

      <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
        <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-1.5">
          <TrendingUp className="w-3.5 h-3.5" /> Energy Providers & Rate Changes
        </div>
        {countyRateActions.length > 0 ? (
          <>
            <TruncatedList
              items={countyRateActions}
              keyFn={(a: RateActionEntry) => a.id}
              initialCount={3}
              render={(a: RateActionEntry) => (
                <>
                  <strong className="text-gray-800">{a.utility}</strong> — {a.actionType || "rate action"}
                  {a.residentialPctIncrease !== null && `, ${a.residentialPctIncrease}% residential increase`}
                  {a.effectiveDate && ` (effective ${a.effectiveDate})`}
                </>
              )}
            />
            {utilityRateHistory.length > 0 && (
              <div className="mt-2 pt-2 border-t border-gray-200">
                <div className="text-[11px] text-gray-500 mb-1">Rate over time:</div>
                <TruncatedList
                  items={utilityRateHistory}
                  keyFn={(h: RateHistoryEntry) => h.id}
                  initialCount={4}
                  render={(h: RateHistoryEntry) => (
                    <>
                      {h.utility} {h.year}: {h.rateCentsPerKwh !== null ? `${h.rateCentsPerKwh}¢/kWh` : "—"}
                      {h.yoyChangePct !== null && ` (${h.yoyChangePct > 0 ? "+" : ""}${h.yoyChangePct}% YoY)`}
                    </>
                  )}
                />
              </div>
            )}
          </>
        ) : (
          <p className="text-xs text-gray-400">
            No rate actions on file for {countyName} specifically — most filings are utility-wide. See the Rate Actions tab for the full statewide list.
          </p>
        )}
      </div>

      {county && (
        <div className="mt-3 text-xs text-gray-400">
          {county.medianHouseholdIncome !== null && <span>County median household income: ${county.medianHouseholdIncome.toLocaleString()}</span>}
          {county.povertyRatePct !== null && <span> · Poverty rate: {county.povertyRatePct}%</span>}
        </div>
      )}

      {places.length > 0 && (
        <div className="mt-3">
          <div className="text-xs font-medium text-gray-700 mb-1.5">
            Places in this county ({places.length})
          </div>
          <TruncatedList
            items={places}
            keyFn={(p) => p.geoid}
            render={(p) => (
              <>
                {p.name}
                {p.population !== null && ` — pop. ${p.population.toLocaleString()}`}
                {p.medianHouseholdIncome !== null && `, $${p.medianHouseholdIncome.toLocaleString()} median income`}
              </>
            )}
          />
        </div>
      )}
    </div>
  );
}  Alcona: [7, 9], Alger: [2, 5], Allegan: [13, 4], Alpena: [6, 9],
  Antrim: [6, 6], Arenac: [9, 8], Baraga: [1, 3], Barry: [13, 5],
  Bay: [10, 9], Benzie: [7, 4], Berrien: [15, 3], Branch: [15, 6],
  Calhoun: [14, 6], Cass: [15, 4], Charlevoix: [5, 5], Cheboygan: [5, 7],
  Chippewa: [3, 7], Clare: [9, 6], Clinton: [12, 7], Crawford: [7, 7],
  Delta: [4, 5], Dickinson: [4, 4], Eaton: [13, 6], Emmet: [5, 6],
  Genesee: [12, 9], Gladwin: [9, 7], Gogebic: [2, 3], "Grand Traverse": [7, 5],
  Gratiot: [11, 6], Hillsdale: [15, 7], Houghton: [1, 2], Huron: [9, 9],
  Ingham: [13, 7], Ionia: [12, 6], Iosco: [8, 9], Iron: [3, 4],
  Isabella: [10, 7], Jackson: [14, 7], Kalamazoo: [14, 5], Kalkaska: [7, 6],
  Kent: [12, 5], Keweenaw: [0, 2], Lake: [9, 4], Lapeer: [11, 9],
  Leelanau: [6, 5], Lenawee: [15, 8], Livingston: [13, 8], Luce: [2, 6],
  Mackinac: [3, 6], Macomb: [13, 10], Manistee: [8, 4], Marquette: [2, 4],
  Mason: [9, 3], Mecosta: [10, 6], Menominee: [5, 4], Midland: [10, 8],
  Missaukee: [8, 6], Monroe: [15, 9], Montcalm: [11, 5], Montmorency: [6, 8],
  Muskegon: [11, 4], Newaygo: [10, 5], Oakland: [13, 9], Oceana: [10, 4],
  Ogemaw: [8, 8], Ontonagon: [1, 1], Osceola: [9, 5], Oscoda: [7, 8],
  Otsego: [6, 7], Ottawa: [12, 4], "Presque Isle": [5, 8], Roscommon: [8, 7],
  Saginaw: [11, 7], Sanilac: [11, 10], Schoolcraft: [3, 5], Shiawassee: [12, 8],
  "St. Clair": [12, 10], "St. Joseph": [15, 5], Tuscola: [11, 8], "Van Buren": [14, 4],
  Washtenaw: [14, 8], Wayne: [14, 9], Wexford: [8, 5],
};

const ALL_COUNTIES = Object.keys(GRID).sort();
const CELL = 42, GAP = 5;
const GRID_ROWS = Math.max(...Object.values(GRID).map(([r]) => r)) + 1;
const GRID_COLS = Math.max(...Object.values(GRID).map(([, c]) => c)) + 1;

const ABBR: Record<string, string> = {
  Alcona: "ALC", Alger: "ALG", Allegan: "ALL", Alpena: "ALP", Antrim: "ANT",
  Arenac: "ARE", Baraga: "BRG", Barry: "BAR", Bay: "BAY", Benzie: "BEN",
  Berrien: "BER", Branch: "BRA", Calhoun: "CAL", Cass: "CAS", Charlevoix: "CHA",
  Cheboygan: "CHE", Chippewa: "CHI", Clare: "CLA", Clinton: "CLI", Crawford: "CRA",
  Delta: "DEL", Dickinson: "DIC", Eaton: "EAT", Emmet: "EMM", Genesee: "GEN",
  Gladwin: "GLA", Gogebic: "GOG", "Grand Traverse": "GT", Gratiot: "GRA",
  Hillsdale: "HIL", Houghton: "HOU", Huron: "HUR", Ingham: "ING", Ionia: "ION",
  Iosco: "IOS", Iron: "IRO", Isabella: "ISA", Jackson: "JAC", Kalamazoo: "KZO",
  Kalkaska: "KLK", Kent: "KEN", Keweenaw: "KEW", Lake: "LAK", Lapeer: "LAP",
  Leelanau: "LEE", Lenawee: "LEN", Livingston: "LIV", Luce: "LUC", Mackinac: "MAK",
  Macomb: "MAC", Manistee: "MAN", Marquette: "MAR", Mason: "MAS", Mecosta: "MEC",
  Menominee: "MEN", Midland: "MID", Missaukee: "MIS", Monroe: "MOE", Montcalm: "MTC",
  Montmorency: "MTM", Muskegon: "MUS", Newaygo: "NEW", Oakland: "OAK", Oceana: "OCE",
  Ogemaw: "OGE", Ontonagon: "ONT", Osceola: "OSC", Oscoda: "OSD", Otsego: "OTS",
  Ottawa: "OTT", "Presque Isle": "PI", Roscommon: "ROS", Saginaw: "SAG",
  Sanilac: "SAN", Schoolcraft: "SCH", Shiawassee: "SHI", "St. Clair": "SC",
  "St. Joseph": "SJ", Tuscola: "TUS", "Van Buren": "VB", Washtenaw: "WAS",
  Wayne: "WAY", Wexford: "WEX",
};

type LayerKey = "broadband" | "water";
type ViewKey = "map" | "list";
type CategoryKey = "both" | "water" | "broadband" | "none";

// ── Broadband: excluding satellite from the headline number ────────────────
// "Any Technology" in the raw FCC BDC data folds in satellite (Starlink et
// al.), which reads as ~100% in nearly every Michigan county/place — it is
// not a meaningful signal of terrestrial accessibility and was producing a
// map/number that looked uniformly excellent everywhere. Confirmed 2026-08-02
// against Ontonagon County specifically: "Any Technology" = 100% in all 9
// sampled places there, while Fixed Wireless is 0% in every one of them and
// Fiber is 0% in three of nine — the real, county-specific story that "Any
// Technology" was masking.
//
// Fix: every broadband number surfaced by this component (map category
// presence, the List view's bar/value, and the county detail card's
// headline) is now computed here directly from the raw per-technology
// records, taking the BEST of the three genuinely terrestrial technologies
// (Fiber, Cable, All Fixed Wireless) per county. "Any Technology" and "All
// Satellite" are never used as the headline; satellite is shown only as an
// explicit, separately-labeled footnote in the detail card so it isn't lost,
// just no longer allowed to stand in for real terrestrial coverage.
//
// NOTE: this only fixes what's computed in THIS component. The hook's own
// `aggregateByCounty` (from useMichiganEssentialServices) may still expose a
// separate, satellite-inclusive `broadbandAvailabilityPct` field — that
// field is intentionally NOT used anywhere below. If other parts of the app
// read that hook field directly, they'll need the same fix applied there.
const FIXED_TERRESTRIAL_TECHS = new Set(["Fiber", "Cable", "All Fixed Wireless"]);

function terrestrialCompositeByCounty(rates: BroadbandRateEntry[]): Map<string, number> {
  const byCounty = new Map<string, Map<string, { total: number; count: number }>>();
  for (const r of rates) {
    if (r.availabilityPctOfZip === null || !FIXED_TERRESTRIAL_TECHS.has(r.technology ?? "")) continue;
    if (!byCounty.has(r.county)) byCounty.set(r.county, new Map());
    const byTech = byCounty.get(r.county)!;
    const entry = byTech.get(r.technology!) ?? { total: 0, count: 0 };
    entry.total += r.availabilityPctOfZip;
    entry.count += 1;
    byTech.set(r.technology!, entry);
  }
  const result = new Map<string, number>();
  for (const [county, byTech] of byCounty) {
    let best = 0;
    for (const { total, count } of byTech.values()) best = Math.max(best, total / count);
    result.set(county, Math.round(best));
  }
  return result;
}

const LAYER_LABEL: Record<LayerKey, string> = {
  broadband: "Broadband Coverage (Terrestrial)",
  water: "Water Affordability Burden",
};
const LAYER_ICON: Record<LayerKey, typeof Wifi> = {
  broadband: Wifi,
  water: Droplets,
};
const VIEW_LABEL: Record<ViewKey, string> = { map: "Map", list: "County List" };

// ── Map coloring: categorical, not continuous ───────────────────────────────
// Previously the map colored counties along a single continuous ramp driven
// by whichever layer (water OR broadband) was toggled on, hiding the other
// layer entirely and making color depend on which button happened to be
// pressed rather than on what data actually exists for that county. Per
// 2026-08-02 discussion, the map now shows a single, fixed categorical view
// of DATA AVAILABILITY across both categories at once — the layer toggle
// (below) still exists for the List view's bar/value, but no longer drives
// the map's fill.
//
// Energy is deliberately excluded from these categories: verified directly
// against the live database that energy_burden_by_race has geography_id =
// null on every row, so it's genuinely statewide reference data, not
// something with a per-county presence/absence to color by. It's shown as
// always-present context in every county's detail panel regardless of color.
// (Note for later: water & sewage could arguably move toward this same
// "always-present statewide context" treatment rather than a hard
// presence/absence category — flagged for a future pass, not done here.)
//
// Colors are the Liberation Caucus palette. Gold/terracotta match values
// already used elsewhere in this file (terracotta = the existing selected-
// county stroke color); forest green is the Pan-African-palette green
// mentioned in brand notes. Confirm against the actual Tailwind config if
// these read as off from the brand's defined hex values.
const CATEGORY_COLOR: Record<CategoryKey, string> = {
  both: "#C9A438",       // gold — water & sewage AND broadband both sampled
  water: "#A8442C",      // terracotta — water & sewage only
  broadband: "#2F6B45",  // forest green — broadband only
  none: "#1a1512",       // near-black — no data yet (a gap, not a "good" score)
};
const CATEGORY_TEXT_COLOR: Record<CategoryKey, string> = {
  both: "#1f2937",
  water: "#f4ecd8",
  broadband: "#f4ecd8",
  none: "#f4ecd8",
};
const CATEGORY_LABEL: Record<CategoryKey, string> = {
  both: "Water & sewage + broadband",
  water: "Water & sewage only",
  broadband: "Broadband only",
  none: "No data yet",
};

function barColor(layer: LayerKey): string {
  return layer === "broadband" ? "hsl(152 55% 55%)" : "hsl(14 65% 58%)";
}

export default function UtilityCountyCartogram() {
  const [view, setView] = useState<ViewKey>("map");
  const [layer, setLayer] = useState<LayerKey>("water");
  const [openCounty, setOpenCounty] = useState<string | null>(null);
  const [hovered, setHovered] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  const { data: servicesData, isLoading: servicesLoading, isError: servicesError } = useMichiganEssentialServices();
  const { data: energyRows, isLoading: energyLoading } = useEnergyBurdenByCounty();

  const isLoading = servicesLoading || energyLoading;
  const isError = servicesError;

  const countiesByName = useMemo(() => {
    const map = new Map<string, CountyEntry>();
    for (const c of servicesData?.counties ?? []) map.set(c.name, c);
    return map;
  }, [servicesData]);

  const countyAggregates = useMemo(
    () => aggregateByCounty(servicesData?.waterSewageRates ?? [], servicesData?.broadbandRates ?? []),
    [servicesData]
  );

  const terrestrialBroadbandByCounty = useMemo(
    () => terrestrialCompositeByCounty(servicesData?.broadbandRates ?? []),
    [servicesData]
  );

  const placesByCounty = useMemo(
    () => groupPlacesByCounty(servicesData?.places ?? []),
    [servicesData]
  );

  function metricValue(countyName: string, ly: LayerKey): number | null {
    if (ly === "broadband") return terrestrialBroadbandByCounty.get(countyName) ?? null;
    return countyAggregates.get(countyName)?.waterAffordabilityRatioPct ?? null;
  }

  function categoryFor(countyName: string): CategoryKey {
    const hasWater = metricValue(countyName, "water") !== null;
    const hasBroadband = metricValue(countyName, "broadband") !== null;
    if (hasWater && hasBroadband) return "both";
    if (hasWater) return "water";
    if (hasBroadband) return "broadband";
    return "none";
  }

  const filteredCounties = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return ALL_COUNTIES;
    return ALL_COUNTIES.filter(
      (n) =>
        n.toLowerCase().includes(q) ||
        (placesByCounty.get(n) ?? []).some((p) => p.name.toLowerCase().includes(q))
    );
  }, [query, placesByCounty]);

  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-6">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 mb-4 pb-0">
        <div className="flex">
          {(Object.keys(VIEW_LABEL) as ViewKey[]).map((key) => (
            <button
              key={key}
              onClick={() => setView(key)}
              className={`px-4 py-2.5 text-[11px] font-bold uppercase tracking-wider border-b-2 -mb-px transition-colors ${
                view === key
                  ? "text-gray-900 border-liberation-gold"
                  : "text-gray-400 border-transparent hover:text-gray-700"
              }`}
              aria-current={view === key ? "true" : undefined}
            >
              {VIEW_LABEL[key]}
            </button>
          ))}
        </div>
        <div className="relative mb-2">
          <Search className="absolute left-2 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
          <input
            type="text"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Find a county…"
            aria-label="Search counties"
            className="pl-7 pr-3 py-1.5 text-xs rounded-md bg-gray-100 border border-gray-300 text-gray-900 placeholder:text-gray-400 w-40 focus:outline-none focus:border-liberation-gold/50"
          />
        </div>
      </div>

      {/* Layer toggle: only meaningful for the List view now, since the map
          uses fixed categorical coloring regardless of layer selection. */}
      {view === "list" && (
        <div className="mb-4">
          <div className="flex gap-2 flex-wrap">
            {(Object.keys(LAYER_LABEL) as LayerKey[]).map((key) => {
              const Icon = LAYER_ICON[key];
              const active = layer === key;
              return (
                <button
                  key={key}
                  onClick={() => setLayer(key)}
                  aria-pressed={active}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border transition-colors ${
                    active
                      ? "bg-liberation-gold text-liberation-dark border-liberation-gold"
                      : "bg-transparent text-gray-600 border-gray-300 hover:border-gray-400"
                  }`}
                >
                  <Icon className="w-3.5 h-3.5" />
                  {LAYER_LABEL[key]}
                </button>
              );
            })}
          </div>
          <p className="text-[11px] text-gray-400 mt-1.5">
            Broadband coverage excludes satellite (near-universal statewide via services like Starlink) — see each county's detail for the satellite figure separately.
          </p>
        </div>
      )}

      {isLoading && (
        <div className="space-y-3" aria-busy="true" aria-label="Loading essential services data">
          <Skeleton className="h-[500px] w-full max-w-2xl rounded-lg" />
          <Skeleton className="h-4 w-2/3" />
          <Skeleton className="h-4 w-1/3" />
        </div>
      )}

      {isError && (
        <div role="alert" className="h-40 flex flex-col items-center justify-center gap-2 text-gray-500 text-sm border border-gray-200 rounded-lg">
          <span>Data couldn't load.</span>
          <button onClick={() => window.location.reload()} className="text-xs text-liberation-gold hover:underline">
            Try refreshing the page
          </button>
        </div>
      )}

      {!isLoading && !isError && view === "map" && (
        <MapView
          query={query}
          openCounty={openCounty}
          setOpenCounty={setOpenCounty}
          hovered={hovered}
          setHovered={setHovered}
          metricValue={metricValue}
          categoryFor={categoryFor}
        />
      )}

      {!isLoading && !isError && view === "list" && (
        <ListView
          layer={layer}
          counties={filteredCounties}
          openCounty={openCounty}
          setOpenCounty={setOpenCounty}
          metricValue={metricValue}
          countiesByName={countiesByName}
          waterRates={servicesData?.waterSewageRates ?? []}
          broadbandRates={servicesData?.broadbandRates ?? []}
          energyRows={energyRows ?? []}
          placesByCounty={placesByCounty}
        />
      )}

      {view === "map" && !isLoading && !isError && (
        <div className="border-t-2 border-gray-200 mt-1">
          {openCounty ? (
            <CountyDetailPanel
              countyName={openCounty}
              county={countiesByName.get(openCounty)}
              waterRates={(servicesData?.waterSewageRates ?? []).filter((w) => w.county === openCounty)}
              broadbandRates={(servicesData?.broadbandRates ?? []).filter((b) => b.county === openCounty)}
              energyRows={energyRows ?? []}
              places={placesByCounty.get(openCounty) ?? []}
              onClose={() => setOpenCounty(null)}
            />
          ) : (
            <p className="text-left text-xs text-gray-400 py-6">
              Select any county above to see its water, energy, and broadband data.
            </p>
          )}
        </div>
      )}
    </div>
  );
}

// ── Map view ──────────────────────────────────────────────────────────────────

function MapView({
  query,
  openCounty,
  setOpenCounty,
  hovered,
  setHovered,
  metricValue,
  categoryFor,
}: {
  query: string;
  openCounty: string | null;
  setOpenCounty: (c: string | null) => void;
  hovered: string | null;
  setHovered: (c: string | null) => void;
  metricValue: (countyName: string, layer: LayerKey) => number | null;
  categoryFor: (countyName: string) => CategoryKey;
}) {
  const q = query.trim().toLowerCase();

  return (
    <>
      <svg
        viewBox={`0 0 ${GRID_COLS * (CELL + GAP)} ${GRID_ROWS * (CELL + GAP)}`}
        className="w-full max-w-2xl block"
        style={{ height: "auto", overflow: "visible" }}
        role="group"
        aria-label="Michigan county cartogram"
      >
        {Object.entries(GRID).map(([name, [r, c]]) => {
          const category = categoryFor(name);
          const isOpen = openCounty === name;
          const isHovered = hovered === name;
          const matches = !q || name.toLowerCase().includes(q);
          const x = c * (CELL + GAP);
          const y = r * (CELL + GAP);
          const cx = x + CELL / 2;
          const cy = y + CELL / 2;
          return (
            <g
              key={name}
              tabIndex={0}
              role="button"
              aria-label={`${name} County — ${CATEGORY_LABEL[category]}`}
              aria-pressed={isOpen}
              opacity={matches ? 1 : 0.2}
              style={{
                cursor: "pointer",
                outline: "none",
                transform: isHovered ? "scale(1.12)" : "scale(1)",
                transformOrigin: `${cx}px ${cy}px`,
                transition: "transform 0.15s ease",
              }}
              onClick={() => setOpenCounty(isOpen ? null : name)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
                  setOpenCounty(isOpen ? null : name);
                }
              }}
              onMouseEnter={() => setHovered(name)}
              onMouseLeave={() => setHovered(null)}
              onFocus={() => setHovered(name)}
              onBlur={() => setHovered(null)}
            >
              <rect
                x={x} y={y} width={CELL} height={CELL} rx={6}
                fill={CATEGORY_COLOR[category]}
                stroke={isOpen ? "#1f2937" : "#e5e7eb"}
                strokeWidth={isOpen ? 2.5 : 0.75}
                style={{ transition: "stroke 0.1s ease" }}
              />
              <text
                x={cx} y={cy}
                textAnchor="middle"
                dominantBaseline="central"
                fontFamily="ui-monospace, monospace"
                fontWeight={600}
                fontSize={ABBR[name]?.length > 2 ? 13 : 15}
                fill={CATEGORY_TEXT_COLOR[category]}
                style={{ pointerEvents: "none" }}
              >
                {ABBR[name] ?? name.slice(0, 3).toUpperCase()}
              </text>
            </g>
          );
        })}
      </svg>

      <div className="text-left text-xs text-gray-400 mt-2 h-4" aria-live="polite">
        {hovered ? (
          <span>
            <strong className="text-gray-800">{hovered} County</strong>
            {" — "}
            {CATEGORY_LABEL[categoryFor(hovered)]}
            {(() => {
              const b = metricValue(hovered, "broadband");
              const w = metricValue(hovered, "water");
              const parts: string[] = [];
              if (b !== null) parts.push(`broadband ${b}% terrestrial`);
              if (w !== null) parts.push(`water ${w}% of income`);
              return parts.length > 0 ? ` (${parts.join(", ")})` : "";
            })()}
          </span>
        ) : (
          "Hover or tab to a county for its data · click or press Enter to open details below"
        )}
      </div>

      <div className="flex gap-4 flex-wrap text-[11px] text-gray-500 py-3">
        {(Object.keys(CATEGORY_LABEL) as CategoryKey[]).map((key) => (
          <span key={key} className="flex items-center gap-1.5">
            <span
              className="inline-block w-3 h-3 rounded-sm border border-gray-300"
              style={{ background: CATEGORY_COLOR[key] }}
            />
            {CATEGORY_LABEL[key]}
          </span>
        ))}
      </div>
    </>
  );
}

// ── County List (Ledger) view ──────────────────────────────────────────────────

function ListView({
  layer,
  counties,
  openCounty,
  setOpenCounty,
  metricValue,
  countiesByName,
  waterRates,
  broadbandRates,
  energyRows,
  placesByCounty,
}: {
  layer: LayerKey;
  counties: string[];
  openCounty: string | null;
  setOpenCounty: (c: string | null) => void;
  metricValue: (countyName: string, layer: LayerKey) => number | null;
  countiesByName: Map<string, CountyEntry>;
  waterRates: WaterSewageRateEntry[];
  broadbandRates: BroadbandRateEntry[];
  energyRows: EnergyBurdenEntry[];
  placesByCounty: Map<string, PlaceEntry[]>;
}) {
  const max = useMemo(() => {
    let m = 1;
    for (const name of counties) {
      const v = metricValue(name, layer);
      if (v !== null) m = Math.max(m, v);
    }
    return m;
  }, [counties, layer, metricValue]);

  if (counties.length === 0) {
    return <p className="text-xs text-gray-400 py-6">No counties or places match that search.</p>;
  }

  return (
    <div role="list" aria-label="Michigan counties">
      {counties.map((name) => {
        const value = metricValue(name, layer);
        const isEmpty = value === null;
        const widthPct = isEmpty ? 0 : Math.max((value / max) * 100, 2);
        const isOpen = openCounty === name;
        const places = placesByCounty.get(name) ?? [];

        return (
          <div key={name} role="listitem" className="border-b border-gray-200">
            <button
              onClick={() => setOpenCounty(isOpen ? null : name)}
              aria-expanded={isOpen}
              className="w-full flex items-center gap-3 py-2.5 text-left"
            >
              <span className={`flex-shrink-0 w-3.5 h-3.5 text-gray-400 transition-transform ${isOpen ? "rotate-90 text-liberation-gold" : ""}`}>
                ▸
              </span>
              <span className={`font-bold text-sm flex-shrink-0 w-28 ${isEmpty ? "text-gray-400" : "text-gray-900"}`}>
                {name}
                {places.length > 0 && (
                  <span className="ml-1.5 text-[10px] font-normal text-gray-400">({places.length})</span>
                )}
              </span>
              <span className="flex-1 h-1.5 rounded-full bg-gray-50 overflow-hidden max-w-[160px]">
                <span className="block h-full rounded-full" style={{ width: `${widthPct}%`, background: barColor(layer) }} />
              </span>
              <span className={`text-xs font-mono w-16 text-right flex-shrink-0 ${isEmpty ? "text-gray-400" : "text-gray-700"}`}>
                {isEmpty ? "—" : `${value}%`}
              </span>
            </button>
            {isOpen && (
              <CountyDetailPanel
                countyName={name}
                county={countiesByName.get(name)}
                waterRates={waterRates.filter((w) => w.county === name)}
                broadbandRates={broadbandRates.filter((b) => b.county === name)}
                energyRows={energyRows}
                places={places}
                onClose={() => setOpenCounty(null)}
                compact
              />
            )}
          </div>
        );
      })}
    </div>
  );
}

// ── County detail panel ──────────────────────────────────────────────────────

function TruncatedList<T>({
  items,
  render,
  keyFn,
  initialCount = 5,
}: {
  items: T[];
  render: (item: T) => ReactNode;
  keyFn: (item: T) => string;
  initialCount?: number;
}) {
  const [expanded, setExpanded] = useState(false);
  const visible = expanded ? items : items.slice(0, initialCount);
  const hiddenCount = items.length - initialCount;

  return (
    <>
      <ul className="space-y-1 text-xs text-gray-600">
        {visible.map((item) => (
          <li key={keyFn(item)}>{render(item)}</li>
        ))}
      </ul>
      {!expanded && hiddenCount > 0 && (
        <button onClick={() => setExpanded(true)} className="text-[11px] text-liberation-gold hover:underline mt-1">
          +{hiddenCount} more
        </button>
      )}
    </>
  );
}

// ── Metric bar list ───────────────────────────────────────────────────────────
// A structured label + horizontal bar + value row, reused for two things that
// don't belong in a pie chart because their values are INDEPENDENT figures
// that don't sum to a meaningful 100%: (1) broadband technology coverage,
// where fiber/cable/fixed-wireless footprints overlap rather than dividing up
// a whole; and (2) energy burden by race, where each group's burden % is its
// own independent statistic. This mirrors the same bold-label +
// structured-metric visual language already used in the Energy Providers &
// Rate Changes list below and in the main county List view's row bars, so
// it's intentional and consistent rather than a bar chart invented from
// nothing.
function MetricBarList({
  items,
  barHexColor,
  emptyLabel = "No data.",
}: {
  items: Array<{ label: string; pct: number; sublabel?: string }>;
  barHexColor: string;
  emptyLabel?: string;
}) {
  if (items.length === 0) return <p className="text-xs text-gray-400">{emptyLabel}</p>;
  const max = Math.max(...items.map((i) => i.pct), 1);
  return (
    <ul className="space-y-1.5">
      {items.map((i) => (
        <li key={i.label} className="flex items-center gap-2">
          <span className="text-[11px] text-gray-700 w-28 flex-shrink-0 truncate" title={i.label}>
            <strong className="font-semibold text-gray-800">{i.label}</strong>
            {i.sublabel && <span className="text-gray-400"> {i.sublabel}</span>}
          </span>
          <span className="flex-1 h-1.5 rounded-full bg-gray-200 overflow-hidden">
            <span
              className="block h-full rounded-full"
              style={{ width: `${Math.min(100, (i.pct / max) * 100)}%`, background: barHexColor }}
            />
          </span>
          <span className="text-[11px] font-mono text-gray-600 w-10 text-right flex-shrink-0">{i.pct}%</span>
        </li>
      ))}
    </ul>
  );
}

// ── Mini pie chart ────────────────────────────────────────────────────────────
// Kept for the two places where slices genuinely ARE parts of a whole
// (sum to ~100%): water bill composition, and population share by race.
const PIE_COLORS = ["hsl(152 45% 58%)", "hsl(14 65% 62%)", "hsl(40 55% 62%)", "hsl(210 45% 62%)", "hsl(280 35% 62%)"];

function MiniPie({ data }: { data: Array<{ name: string; value: number }> }) {
  if (data.length === 0 || data.every((d) => d.value === 0)) {
    return <p className="text-xs text-gray-400">Not enough data to chart.</p>;
  }
  return (
    <div className="flex items-center gap-3">
      <div style={{ width: 72, height: 72 }} className="flex-shrink-0">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} dataKey="value" nameKey="name" innerRadius={18} outerRadius={34} paddingAngle={2}>
              {data.map((_, i) => (
                <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} stroke="none" />
              ))}
            </Pie>
            <Tooltip
              formatter={(value: number, name: string) => [`${value}%`, name]}
              contentStyle={{ background: "#1a1512", border: "1px solid rgba(244,236,216,0.15)", borderRadius: 6, fontSize: 11 }}
              itemStyle={{ color: "#f4ecd8" }}
            />
          </PieChart>
        </ResponsiveContainer>
      </div>
      <ul className="text-[11px] text-gray-600 space-y-0.5">
        {data.map((d, i) => (
          <li key={d.name} className="flex items-center gap-1.5">
            <span className="inline-block w-2 h-2 rounded-full flex-shrink-0" style={{ background: PIE_COLORS[i % PIE_COLORS.length] }} />
            {d.name}: {d.value}%
          </li>
        ))}
      </ul>
    </div>
  );
}

function CountyDetailPanel({
  countyName,
  county,
  waterRates,
  broadbandRates,
  energyRows,
  places,
  onClose,
  compact = false,
}: {
  countyName: string;
  county: CountyEntry | undefined;
  waterRates: WaterSewageRateEntry[];
  broadbandRates: BroadbandRateEntry[];
  energyRows: EnergyBurdenEntry[];
  places: PlaceEntry[];
  onClose: () => void;
  compact?: boolean;
}) {
  const { data: allRateActions } = useRateActionsByCounty() as { data: RateActionEntry[] | undefined };
  const { data: allRateHistory } = useRateHistoryByCounty() as { data: RateHistoryEntry[] | undefined };

  const countyRateActions = useMemo(
    () => (allRateActions ?? []).filter((a: RateActionEntry) => a.geography === countyName),
    [allRateActions, countyName]
  );

  const relevantUtilities = useMemo(
    () => new Set(countyRateActions.map((a: RateActionEntry) => a.utility).filter(Boolean)),
    [countyRateActions]
  );
  const utilityRateHistory = useMemo(
    () =>
      (allRateHistory ?? [])
        .filter((h: RateHistoryEntry) => relevantUtilities.has(h.utility))
        .sort((a: RateHistoryEntry, b: RateHistoryEntry) => (a.year ?? 0) - (b.year ?? 0)),
    [allRateHistory, relevantUtilities]
  );

  // Broadband: average availability per technology, split into terrestrial
  // (the headline), satellite (a footnote — near-universal, not a meaningful
  // differentiator), and mobile (its own small section, if present — mobile
  // rows measure LAND AREA covered, not housing units, so they are never
  // mixed into the terrestrial housing-unit figures above).
  const broadbandByTech = useMemo(() => {
    const sums = new Map<string, { total: number; count: number }>();
    for (const r of broadbandRates) {
      if (r.availabilityPctOfZip === null) continue;
      const tech = r.technology || "Unknown";
      const entry = sums.get(tech) ?? { total: 0, count: 0 };
      entry.total += r.availabilityPctOfZip;
      entry.count += 1;
      sums.set(tech, entry);
    }
    return Array.from(sums.entries()).map(([tech, { total, count }]) => ({
      tech,
      avgPct: Math.round(total / count),
    }));
  }, [broadbandRates]);

  const terrestrialTechs = broadbandByTech.filter((t) => FIXED_TERRESTRIAL_TECHS.has(t.tech));
  const satelliteTech = broadbandByTech.find((t) => t.tech === "All Satellite");
  const mobileTechs = broadbandByTech.filter((t) => t.tech.startsWith("Mobile"));
  const terrestrialHeadline =
    terrestrialTechs.length > 0 ? Math.max(...terrestrialTechs.map((t) => t.avgPct)) : null;

  const waterPie = useMemo(() => {
    const withBoth = waterRates.filter(
      (r) => r.estimatedTypicalMonthlyBill !== null && r.customerChargeMonthly !== null && r.estimatedTypicalMonthlyBill > 0
    );
    if (withBoth.length === 0) return [];
    const avgBill = withBoth.reduce((s, r) => s + (r.estimatedTypicalMonthlyBill ?? 0), 0) / withBoth.length;
    const avgFixed = withBoth.reduce((s, r) => s + (r.customerChargeMonthly ?? 0), 0) / withBoth.length;
    const fixedPct = Math.max(0, Math.min(100, Math.round((avgFixed / avgBill) * 100)));
    return [
      { name: "Fixed charge", value: fixedPct },
      { name: "Est. usage charge", value: 100 - fixedPct },
    ];
  }, [waterRates]);

  const racePopulationPie = useMemo(() => {
    if (!county || county.raceBreakdown.length === 0) return [];
    return county.raceBreakdown
      .filter((r) => r.pctOfPopulation !== null)
      .map((r) => ({ name: r.category, value: r.pctOfPopulation as number }));
  }, [county]);

  // Energy burden by race: previously a plain text list of independent
  // percentages (which don't sum to 100, so were never suited to a pie).
  // Now a bar list — same structured label + bar + value treatment used for
  // broadband above and for the Rate Changes list below, so a reader can
  // compare groups at a glance instead of reading numbers off a list.
  const burdenBarItems = useMemo(
    () =>
      energyRows
        .filter((r) => r.medianBurdenPct !== null)
        .map((r) => ({
          label: r.racialGroup,
          pct: r.medianBurdenPct as number,
          sublabel: r.dataYear ? `(${r.dataYear})` : undefined,
        })),
    [energyRows]
  );

  return (
    <div className={compact ? "pb-4 pl-6" : "py-4"} aria-live="polite">
      {!compact && (
        <div className="flex items-center justify-between mb-3">
          <h4 className="font-semibold text-gray-900">{countyName} County</h4>
          <button onClick={onClose} className="text-xs text-gray-400 hover:text-gray-800">
            Close ✕
          </button>
        </div>
      )}

      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Wifi className="w-3.5 h-3.5" /> Broadband
          </div>
          {broadbandRates.length > 0 ? (
            <>
              {terrestrialHeadline !== null && (
                <div className="text-2xl font-bold text-gray-900 mb-1">
                  {terrestrialHeadline}%
                  <span className="text-[10px] font-normal text-gray-400 ml-1.5">best terrestrial option, avg.</span>
                </div>
              )}
              <MetricBarList
                items={terrestrialTechs.map((t) => ({ label: t.tech, pct: t.avgPct }))}
                barHexColor="#2F6B45"
                emptyLabel="No terrestrial (fiber/cable/fixed wireless) data sampled."
              />
              {satelliteTech && (
                <p className="text-[11px] text-gray-400 mt-2 pt-2 border-t border-gray-200">
                  Satellite: {satelliteTech.avgPct}% — omitted from the figure above; typically costlier and higher-latency than terrestrial options, and near-universal statewide.
                </p>
              )}
              {mobileTechs.length > 0 && (
                <div className="mt-2 pt-2 border-t border-gray-200">
                  <div className="text-[11px] text-gray-500 mb-1">
                    Mobile coverage (share of land area, not housing units):
                  </div>
                  <MetricBarList
                    items={mobileTechs.map((t) => ({ label: t.tech.replace("Mobile ", ""), pct: t.avgPct }))}
                    barHexColor="hsl(210 45% 55%)"
                  />
                </div>
              )}
            </>
          ) : (
            <p className="text-xs text-gray-400">Not yet sampled in this county.</p>
          )}
        </div>

        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Droplets className="w-3.5 h-3.5" /> Water & Sewage
          </div>
          {waterRates.length > 0 ? (
            <>
              <MiniPie data={waterPie} />
              <div className="mt-2">
                <TruncatedList
                  items={waterRates}
                  keyFn={(r) => r.id}
                  initialCount={3}
                  render={(r) => (
                    <>
                      {r.provider} ({r.municipalityServiceArea || "—"})
                      {r.estimatedTypicalMonthlyBill !== null && ` — $${r.estimatedTypicalMonthlyBill}/mo`}
                      {r.affordabilityRatioPct !== null && `, ${r.affordabilityRatioPct}% of income`}
                    </>
                  )}
                />
              </div>
            </>
          ) : (
            <p className="text-xs text-gray-400">Not yet sampled in this county.</p>
          )}
        </div>

        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-2">
            <Zap className="w-3.5 h-3.5" /> Energy Burden by Race
          </div>
          {racePopulationPie.length > 0 && (
            <>
              <div className="text-[11px] text-gray-500 mb-1">{countyName} population by race:</div>
              <MiniPie data={racePopulationPie} />
            </>
          )}
          {burdenBarItems.length > 0 ? (
            <div className="mt-2 pt-2 border-t border-gray-200">
              <div className="text-[11px] text-gray-500 mb-1.5">
                Statewide reference (not specific to {countyName} — energy burden isn't tracked at the county level in this data source):
              </div>
              <MetricBarList items={burdenBarItems} barHexColor="#A8442C" />
            </div>
          ) : (
            <p className="text-xs text-gray-400 mt-2">Statewide energy burden reference data unavailable.</p>
          )}
        </div>
      </div>

      <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
        <div className="flex items-center gap-1.5 text-xs font-medium text-gray-700 mb-1.5">
          <TrendingUp className="w-3.5 h-3.5" /> Energy Providers & Rate Changes
        </div>
        {countyRateActions.length > 0 ? (
          <>
            <TruncatedList
              items={countyRateActions}
              keyFn={(a: RateActionEntry) => a.id}
              initialCount={3}
              render={(a: RateActionEntry) => (
                <>
                  <strong className="text-gray-800">{a.utility}</strong> — {a.actionType || "rate action"}
                  {a.residentialPctIncrease !== null && `, ${a.residentialPctIncrease}% residential increase`}
                  {a.effectiveDate && ` (effective ${a.effectiveDate})`}
                </>
              )}
            />
            {utilityRateHistory.length > 0 && (
              <div className="mt-2 pt-2 border-t border-gray-200">
                <div className="text-[11px] text-gray-500 mb-1">Rate over time:</div>
                <TruncatedList
                  items={utilityRateHistory}
                  keyFn={(h: RateHistoryEntry) => h.id}
                  initialCount={4}
                  render={(h: RateHistoryEntry) => (
                    <>
                      {h.utility} {h.year}: {h.rateCentsPerKwh !== null ? `${h.rateCentsPerKwh}¢/kWh` : "—"}
                      {h.yoyChangePct !== null && ` (${h.yoyChangePct > 0 ? "+" : ""}${h.yoyChangePct}% YoY)`}
                    </>
                  )}
                />
              </div>
            )}
          </>
        ) : (
          <p className="text-xs text-gray-400">
            No rate actions on file for {countyName} specifically — most filings are utility-wide. See the Rate Actions tab for the full statewide list.
          </p>
        )}
      </div>

      {county && (
        <div className="mt-3 text-xs text-gray-400">
          {county.medianHouseholdIncome !== null && <span>County median household income: ${county.medianHouseholdIncome.toLocaleString()}</span>}
          {county.povertyRatePct !== null && <span> · Poverty rate: {county.povertyRatePct}%</span>}
        </div>
      )}

      {places.length > 0 && (
        <div className="mt-3">
          <div className="text-xs font-medium text-gray-700 mb-1.5">
            Places in this county ({places.length})
          </div>
          <TruncatedList
            items={places}
            keyFn={(p) => p.geoid}
            render={(p) => (
              <>
                {p.name}
                {p.population !== null && ` — pop. ${p.population.toLocaleString()}`}
                {p.medianHouseholdIncome !== null && `, $${p.medianHouseholdIncome.toLocaleString()} median income`}
              </>
            )}
          />
        </div>
      )}
    </div>
  );
}
